...
$query = "SELECT * FROM transit_hub WHERE 1";
...
while ($row=mysqli_fetch_assoc($result)){
  ...
  $txt =  "\n <trans ";
 fwrite($myfile, $txt);
  $txt = 'coorx="' . $row['TRANS_COORX'] . '" ';
 fwrite($myfile, $txt);
  $txt = 'coory="' . parseToXML($row['TRANS_COORY']) . '" ';
 fwrite($myfile, $txt);
  $txt = 'id="' . parseToXML($row['TRANS_ID']) . '" ';
 fwrite($myfile, $txt);
  $txt =  'name="' . $row['ONOMA_TRANS'] . '" ';
 fwrite($myfile, $txt);
  $txt = '/>';
 fwrite($myfile, $txt);
}
